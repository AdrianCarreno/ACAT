...
class TestDeleteUnused:
    def test_success(self, runner: CliRunner, template_file: str, path_prefix="/test1"):
    ...
    def test_success_no_parameters_to_delete(
        self, runner: CliRunner, template_file: str
    ):
        result = runner.invoke(
    ...
    def test_success_aborted(self, runner: CliRunner, template_file: str):
        result = runner.invoke(
    ...
    def test_fail_without_providing_path_preffix(self, runner: CliRunner):
        result = runner.invoke(delete_unused, ["-t", "template.yaml"])

        assert result.exit_code == 2
        assert "Error: Missing argument 'PATH_PREFFIX'." in result.output

    def test_fail_template_file_not_found(self, runner: CliRunner):
    ...
    def test_fail_template_file_invalid_type(self, runner: CliRunner):
    ...
    def test_fail_template_file_no_parameters(
        self, runner: CliRunner, template_file_invalid_content: str
    ):
        result = runner.invoke(
...
class TestCopy:
    def test_success(self, runner: CliRunner, source="/test1", destination="/test2"):
    ...
    def test_success_no_parameters_to_copy(self, runner: CliRunner):
        result = runner.invoke(copy, ["/no-path", "/test2"], input="y\n")

        assert result.exit_code == 0
        assert "No parameters to copy" in result.output

    def test_success_aborted(self, runner: CliRunner):
